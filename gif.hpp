...
// write a 256-color (8-bit) image palette to the file
void GifPalette::GifWritePalette( FILE* f )
{
    fputc(0, f);  // first color: transparency
    fputc(0, f);
    fputc(0, f);

    for(int ii=1; ii<(1 << bitDepth); ++ii)
    {
        fputc(r[ii], f);
        fputc(g[ii], f);
        fputc(b[ii], f);
...
// only changes them if it finds a better color in its subtree.
// this is the major hotspot in the code at the moment.
void GifPalette::GifGetClosestPaletteColor(int r, int g, int b, int& bestInd, int& bestDiff, int treeRoot)
{
    GifPalette* pPal = this;
    ...
    int splitComp = comps[treeSplitElt[treeRoot]];

    int splitPos = treeSplit[treeRoot];
...
void GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    ...
    for( uint32_t yy=0; yy<height; ++yy )
    {
        for( uint32_t xx=0; xx<width; ++xx )
        {
            ...
            // Search the palete
            pPal->GifGetClosestPaletteColor(rr, gg, bb, bestInd, bestDiff);
...
void GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    ...
    for( uint32_t ii=0; ii<numPixels; ++ii )
    {
        ...
        else
        {
            ...
            int32_t bestDiff = 1000000;
            int32_t bestInd = 1;
            pPal->GifGetClosestPaletteColor(nextFrame[0], nextFrame[1], nextFrame[2], bestInd, bestDiff);
...
private:
    ...
    bool GifBegin( const char* filename, uint32_t width, uint32_t height, uint32_t delay=100, int32_t bitDepth = 8, bool dither = false );
    ...
    bool GifWriteFrame( const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay=100, int bitDepth = 8, bool dither = false );
...
// AFAIK, it is legal to use different bit depths for different frames of an image -
// this may be handy to save bits in animations that don't change much.
bool GifWriter::GifWriteFrame( const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth, bool dither)
{
    GifWriter* writer = this;
    if(!writer->f) return false;
